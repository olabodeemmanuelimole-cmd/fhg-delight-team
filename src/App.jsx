...
function ModulePage({ type, onBack, onNavigate, onSelectOffice, onSelectMember, onReviewPlan, onEditOrder, onEditFinance, onViewFinanceHistory, selectedOfficeId, selectedMemberId, selectedTransactionId, user }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (type === 'activity') {
        ...
        rows = transactions.map(item => [item.description, `${item.book} · ${item.currency} · ${new Date(`${item.transaction_date}T00:00:00`).toLocaleDateString()}`, `${item.entry_type === 'credit' ? '+' : '-'}${formatCurrency(item.amount,item.currency)}`, item.entry_type, item.id, item.owner_id, item.updated_at !== item.created_at])
  ...
  return <main className="detail-screen"><header className="detail-header"><button className="icon-button" aria-label="Go back" onClick={onBack}><ArrowLeft /></button><div><h1>{data.title}</h1><span>{liveSubtitle}</span></div><button className="icon-button" aria-label="Open notifications" onClick={() => onNavigate(type.startsWith('admin') ? 'adminnotifications' : 'notifications')}><Bell /></button></header><div className="detail-content">
    ...
    {type === 'books' && <section className="records finance-actions"><div className="section-title"><h2>{recordScope==='mine'?'Your transactions':'Office transactions'}</h2>{filterable&&<button onClick={()=>{setFilterOpen(value=>!value);if(filterOpen)setFilterQuery('')}}>{filterOpen?'Close':'Filter'}</button>}</div>{filterOpen&&<input className="record-filter" value={filterQuery} onChange={event=>setFilterQuery(event.target.value)} placeholder="Search transactions" autoFocus/>}{visibleRows.length?visibleRows.map(([title,meta,value,status,recordId,recordOwnerId,wasEdited])=><article key={recordId}><span className={`record-symbol ${status}`}><BookOpen /></span><div><strong>{title}</strong><small>{meta}{wasEdited?' · Edited':''}</small><b>{value}</b></div>{recordOwnerId===user.id?<span className="row-actions"><button onClick={()=>onEditFinance(recordId)}>Edit</button><button onClick={()=>onViewFinanceHistory(recordId)}>History</button><button className="danger" onClick={()=>deleteFinanceTransaction(recordId)}>Delete</button></span>:<span className="record-value"><small>View only</small></span>}</article>):<div className="live-empty">No Finance entries yet. Add your first income or expense.</div>}</section>}
