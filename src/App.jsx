  ...
  useEffect(() => {
    ...
    const loadSummary = async () => {
      ...
      const [ordersResult, plansResult, attendanceResult, booksResult] = await Promise.all([
        supabase.from('orders').select('project_name,amount,platform,status,created_at').eq('user_id',user.id).gte('created_at',monthStart.toISOString()).lt('created_at',monthEnd.toISOString()).order('created_at',{ascending:false}),
      ...
      const completedEarnings = orderRows.filter(item => item.status === 'completed').reduce((sum,item) => sum + Number(item.amount),0)
...
const moduleActionTargets = {
  activity: 'addorder', attendanceregister: 'attendance',
  ...
  leaderdashboard: 'createannouncement', feedbackinbox: 'sendfeedback',
  admindashboard: 'addoffice', offices: 'addoffice', leaders: 'assignleader', transfers: 'transfer', profile: 'editprofile', settings: 'editprofile', adminsettings: 'editprofile', announcementtargeting: 'createannouncement',
}

function ModulePage({ type, onBack, onNavigate, onSelectOffice, onSelectMember, onReviewPlan, selectedOfficeId, selectedMemberId, user }) {
  ...
  const liveOperationTypes = ['team','memberprofile','withdrawals','transfers','transferapprovals','reports','teamreports','auditlog','profile','settings','adminsettings']
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (type === 'activity') {
        ...
        let query = supabase.from('orders').select('id,user_id,project_name,amount,platform,status,created_at,profiles(full_name)').order('created_at',{ascending:false})
        if (recordScope === 'mine') query = query.eq('user_id', user.id)
        const result = await query
        error = result.error
        const dollars = amount => `$${Number(amount || 0).toLocaleString('en-US', {maximumFractionDigits:2})}`
        rows = (result.data || []).map(item => [item.project_name, `${recordScope === 'team' ? `${item.profiles?.full_name || 'Team member'} · ` : ''}${item.platform} · ${new Date(item.created_at).toLocaleDateString()}`, dollars(item.amount), item.status, item.id, item.user_id])
        const gross = (result.data || []).reduce((sum,item) => sum + Number(item.amount),0)
        const completed = (result.data || []).filter(item => item.status === 'completed').reduce((sum,item) => sum + Number(item.amount),0)
        metrics = [['Gross orders',dollars(gross)],['Completed earnings',dollars(completed)]]
      } else if (type === 'plans') {
        ...
        let query = supabase.from('cash_books').select('id,owner_id,name,currency,visibility,cash_transactions(entry_type,amount,description,transaction_date,created_at)').order('created_at',{ascending:false})
        ...
        const result = await query
        error = result.error
        const transactions = (result.data || []).flatMap(book => (book.cash_transactions || []).map(item => ({...item,book:book.name,currency:book.currency}))).sort((a,b) => new Date(b.created_at)-new Date(a.created_at))
        ...
        rows = transactions.map(item => [item.description, `${item.book} · ${item.currency} · ${new Date(`${item.transaction_date}T00:00:00`).toLocaleDateString()}`, `${item.entry_type === 'credit' ? '+' : '-'}${formatCurrency(item.amount,item.currency)}`, item.entry_type])
    ...
    load()
    return () => { cancelled = true }
  }, [type, refreshKey, recordScope, user.id, user.ledOfficeId, activeTab, selectedOfficeId, selectedMemberId])
  ...
  const updateOrderStatus = async (orderId, status) => {
    setMessage(`${status === 'completed' ? 'Completing' : 'Cancelling'} order…`)
    const { error } = await supabase.from('orders').update({ status, updated_at:new Date().toISOString() }).eq('id',orderId).eq('user_id',user.id)
...
function FormPage({ type, onBack, selectedPlanId, user }) {
  ...
  const submitForm = async event => {
    ...
    if (type === 'editprofile') {
      ...
      ;({ error } = await supabase.from('orders').insert({ user_id:userId, project_name:value('Project name'), amount:numericAmount('Amount (USD)'), platform:value('Platform'), status:value('Status').toLowerCase(), notes:value('Notes') || null }))
