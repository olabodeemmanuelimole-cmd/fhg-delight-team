...
const screenData = {
  ...
  anonymousinbox: {
    title: 'Anonymous messages', subtitle: 'Identity-free messages from office members', action: 'Refresh inbox', icon: Bell,
    heroLabel: 'Protected inbox', listTitle: 'Anonymous messages',
    metrics: [['Messages','0'],['New','0']], tabs: ['All','New'], rows: [],
    note: ['Sender identity is not collected','Only the originating office, chosen recipient route, message and submission time are stored.']
  },
  events: {
  ...
  leaderdashboard: {
    title: 'Leader dashboard', subtitle: 'Office performance and weekly records', action: 'Record weekly order', icon: TrendUp,
  ...
  weeklyorders: {
    title: 'Weekly office orders', subtitle: 'Orders recorded during the current week', action: 'Add order', icon: Briefcase,
    heroLabel: 'This week', listTitle: 'Office order register',
    metrics: [['Orders','0'],['Gross value','$0']], tabs: ['This week','Completed'], rows: [],
    note: ['One record, automatic weekly totals','Orders are recorded once. TeamFlow groups them into the correct week using their recorded date.']
  },
  members: {
...
const moduleActionTargets = {
  ...
  announcements: 'createannouncement', feedback: 'sendfeedback', events: 'createevent', anonymousinbox:'anonymousinbox',
  leaderdashboard: 'addorder', weeklyorders:'addorder', feedbackinbox: 'createannouncement', memberprofile:'createannouncement',
...
function ModulePage({ type, onBack, onNavigate, onSelectOffice, onSelectMember, onSelectBook = bookId => { window.sessionStorage.setItem('teamflowSelectedBook',bookId); onNavigate('bookdetail') }, onReviewPlan, onEditOrder, onEditFinance, onViewFinanceHistory, selectedOfficeId, selectedMemberId, selectedBookId, selectedTransactionId, user }) {
  ...
  const [pendingRankRequest, setPendingRankRequest] = useState(null)
  const [leaderAccessSelection, setLeaderAccessSelection] = useState('limited')
  const effectiveBookId=selectedBookId||window.sessionStorage.getItem('teamflowSelectedBook')
  const liveAdminTypes = ['admindashboard','leaderdashboard','weeklyorders','offices','officedetail','leaders','orgmembers','members','attendanceregister','roles']
  const liveEngagementTypes = ['announcements','feedback','feedbackinbox','anonymousinbox','events','rewards','pointsettings','notifications','adminnotifications']
  ...
  useEffect(() => {
    if (!selectedDirectoryMember || !supabase) return
    supabase.from('ranks').select('name').eq('active',true).order('sort_order').then(({data,error}) => {
      if (error) setMessage(error.message)
      else setRankOptions((data||[]).map(item=>item.name))
    })
    setRankSelection(selectedDirectoryMember[1].split(' · ').at(-1))
    setLeaderAccessSelection(selectedDirectoryMember[6] || 'limited')
    setRankReason('')
    setPendingRankRequest(null)
    if (user.role==='Administrator') {
      supabase.from('rank_change_requests').select('id,old_rank,new_rank,reason').eq('member_id',selectedDirectoryMember[4]).eq('status','pending').maybeSingle().then(({data})=>setPendingRankRequest(data||null))
    }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (type === 'activity') {
      ...
      } else if (type === 'weeklyorders') {
        const now=new Date(); const monday=new Date(now); monday.setHours(0,0,0,0); monday.setDate(now.getDate()-((now.getDay()+6)%7)); const nextMonday=new Date(monday); nextMonday.setDate(monday.getDate()+7)
        ...
        const result=memberIds.length ? await supabase.from('orders').select('id,user_id,project_name,amount,fee_percent,status,created_at,profiles(full_name)').in('user_id',memberIds).gte('created_at',monday.toISOString()).lt('created_at',nextMonday.toISOString()).order('created_at',{ascending:false}) : {data:[],error:null}
        error=membershipsResult.error||result.error
        const orders=result.data||[]; const gross=orders.reduce((sum,item)=>sum+Number(item.amount||0),0); const completed=orders.filter(item=>item.status==='completed'); const net=completed.reduce((sum,item)=>sum+Number(item.amount||0)*(1-Number(item.fee_percent||0)/100),0)
        rows=orders.map(item=>[item.project_name,`${item.profiles?.full_name||'Office member'} · ${new Date(item.created_at).toLocaleDateString()}`,dollars(item.amount),item.status,item.id,item.user_id])
        metrics=[['Orders this week',String(orders.length)],['Gross / completed net',`${dollars(gross)} / ${dollars(net)}`]]
      } else if (type === 'plans') {
      ...
      } else if (type === 'anonymousinbox') {
        const result=await supabase.rpc('get_anonymous_messages')
        error=result.error
        const messages=result.data||[]
        rows=messages.map(item=>[item.message,`${item.office_name} · ${new Date(item.created_at).toLocaleString()}`,item.recipient_scope==='both'?'Office leader + overall director':item.recipient_scope==='office_leader'?'Office team leader':'Overall director',item.status,item.id])
        metrics=[['Messages',String(messages.length)],['New',String(messages.filter(item=>item.status==='new').length)]]
      } else if (type === 'events') {
        ...
        const result = await supabase.from('rank_change_requests').select('id,member_id,old_rank,new_rank,reason,status,created_at,member:profiles!rank_change_requests_member_id_fkey(full_name)').eq('status','pending').order('created_at',{ascending:false})
        error=result.error
        const requests=result.data||[]
        rows=requests.map(item=>[item.member?.full_name||'Member',`${item.old_rank} → ${item.new_rank}${item.reason?` · ${item.reason}`:''}`,new Date(item.created_at).toLocaleDateString(),'Pending',item.id,item.member_id,'rank-request'])
        metrics=[['Pending requests',String(requests.length)],['Approval level','Administrator']]
        ...
        const [profilesResult, membershipsResult, invitationsResult] = await Promise.all([
          supabase.from('profiles').select('id,full_name,rank,role,status,leader_access_level,created_at').order('full_name'),
          supabase.from('office_memberships').select('user_id,office:offices(id,name)').is('ended_at',null),
          type === 'leaders' && user.role === 'Administrator' ? supabase.from('team_leader_invitations').select('id,status,claimed_by,access_level,created_at,office:offices(id,name),claimant:profiles!team_leader_invitations_claimed_by_fkey(full_name,rank)').in('status',['pending','claimed']).order('created_at',{ascending:false}) : Promise.resolve({data:[],error:null}),
        ])
        ...
        rows = profiles.map(item => [item.full_name, `${officeByUser[item.id] || 'Office pending'} · ${item.rank}`, item.role === 'team_leader' ? `Team leader · ${item.leader_access_level || 'limited'} access` : item.role === 'admin' ? 'Administrator' : 'Member', item.status, item.id, null, item.role === 'team_leader' ? item.leader_access_level || 'limited' : null])
        if (type === 'leaders') rows = [...(invitationsResult.data || []).map(item => [item.claimant?.full_name || 'Invitation not claimed', `${item.office?.name || 'Office'} · ${item.claimant?.rank || 'Registration link sent'}`, item.claimed_by ? `${item.access_level || 'limited'} access requested` : `${item.access_level || 'limited'} access invitation`, 'Pending', item.id, item.claimed_by, 'leader-invite']), ...rows]
  ...
  const visibleRows = displayRows.filter(row => {
    ...
    if (type === 'weeklyorders') return activeTab === 0 || row[3] === 'completed'
    ...
    if (type === 'anonymousinbox') return activeTab === 0 || row[3] === 'new'
  ...
  const displayTabs = type === 'orders' ? [`All ${displayRows.length}`,`Active ${displayRows.filter(row => row[3] === 'active').length}`,`Completed ${displayRows.filter(row => row[3] === 'completed').length}`]
    : type === 'weeklyorders' ? [`This week ${displayRows.length}`,`Completed ${displayRows.filter(row=>row[3]==='completed').length}`]
    ...
    : type === 'anonymousinbox' ? [`All ${displayRows.length}`,`New ${displayRows.filter(row=>row[3]==='new').length}`]
  ...
  const setMemberStatus = async (memberId, status) => {
    const memberName = selectedDirectoryMember?.[0] || 'this member'
    if (status === 'suspended' && !window.confirm(`Suspend ${memberName}? They will lose active access until you approve them again.`)) return
    ...
    if (!error) { setSelectedDirectoryMember(null); setRefreshKey(value=>value+1) }
  }
  ...
  const submitRankChange = async () => {
    if (!selectedDirectoryMember || !rankSelection) return
    const currentRank=selectedDirectoryMember[1].split(' · ').at(-1)
    if (rankSelection===currentRank) { setMessage('Select a different rank.'); return }
    if (!window.confirm(`Change ${selectedDirectoryMember[0]}'s rank from ${currentRank} to ${rankSelection}?`)) return
    setMessage(user.role==='Administrator'?'Updating member rank…':'Sending rank change for approval…')
    const {data,error}=await supabase.rpc('request_rank_change',{target_user_id:selectedDirectoryMember[4],requested_rank:rankSelection,change_reason:rankReason.trim()||null})
    setMessage(error?error.message:data==='applied'?'Member rank updated and notification sent.':'Rank change request sent to the administrator.')
    if(!error){setSelectedDirectoryMember(null);setRefreshKey(value=>value+1)}
  }
  const decideRankChange = async (requestId,decision) => {
    if (decision==='rejected' && !window.confirm('Reject this rank change request?')) return
    setMessage(`${decision==='approved'?'Approving':'Rejecting'} rank change…`)
    const {error}=await supabase.rpc('admin_decide_rank_change',{request_id:requestId,decision})
    setMessage(error?error.message:`Rank change ${decision}. The member has been notified.`)
    if(!error)setRefreshKey(value=>value+1)
  }
  ...
  const updateAnonymousStatus = async (messageId,status) => {
    setMessage(`Marking anonymous message ${status}…`)
    const {error}=await supabase.rpc('update_anonymous_message_status',{message_id:messageId,new_status:status})
    setMessage(error?error.message:`Anonymous message marked ${status}.`)
    if(!error)setRefreshKey(value=>value+1)
  }
  ...
  const changeLeaderAccess = async () => {
    if (!selectedDirectoryMember || user.role !== 'Administrator') return
    if (!window.confirm(`Change ${selectedDirectoryMember[0]} to ${leaderAccessSelection} team-leader access?`)) return
    setMessage('Updating team-leader access…')
    const {error}=await supabase.rpc('admin_set_team_leader_access',{target_user_id:selectedDirectoryMember[4],new_access_level:leaderAccessSelection})
    setMessage(error?error.message:`Team-leader access changed to ${leaderAccessSelection}.`)
    if(!error){setSelectedDirectoryMember(null);setRefreshKey(value=>value+1)}
  }
  const runAction = async () => {
    ...
    if (type === 'anonymousinbox') { setRefreshKey(value=>value+1); setMessage('Anonymous inbox refreshed.'); return }
  ...
  const filterable = ['activity','orders','weeklyorders','plans','books','attendance','withdrawals','team','announcements','feedback','events','notifications','adminnotifications','anonymousinbox','members','attendanceregister','transferapprovals','feedbackinbox','admindashboard','offices','leaders','orgmembers','transfers','reports','teamreports','auditlog','pointsettings'].includes(type)
  return <main className="detail-screen"><header className="detail-header"><button className="icon-button" aria-label="Go back" onClick={onBack}><ArrowLeft /></button><div><h1>{data.title}</h1><span>{liveSubtitle}</span></div><button className="icon-button" aria-label="Open notifications" onClick={() => onNavigate(type.startsWith('admin') ? 'adminnotifications' : 'notifications')}><Bell /></button></header><div className="detail-content">
    ...
    {type === 'admindashboard' && <div className="admin-shortcuts"><button onClick={() => onNavigate('offices')}><Briefcase />Offices</button><button onClick={() => onNavigate('leaders')}><Users />Leaders</button><button onClick={() => onNavigate('orgmembers')}><Users />Members</button><button onClick={() => onNavigate('anonymousinbox')}><Bell />Anonymous inbox</button><button onClick={() => onNavigate('transfers')}><ArrowRight />Transfers</button><button onClick={() => onNavigate('reports')}><TrendUp />Reports</button><button onClick={() => onNavigate('pointsettings')}><Sparkle />Rewards</button><button onClick={() => onNavigate('auditlog')}><CheckCircle />Audit</button></div>}
    ...
    <section className="records"><div className="section-title"><h2>{data.listTitle || (type === 'plans' ? (recordScope === 'team' ? 'Team submissions' : 'Your plan') : type === 'attendance' ? (recordScope === 'team' ? 'Office register' : 'Daily history') : 'Recent records')}</h2>{filterable && <button onClick={() => { setFilterOpen(value=>!value); if(filterOpen)setFilterQuery('') }}>{filterOpen?'Close':'Filter'}</button>}</div>{filterOpen && filterable && <input className="record-filter" value={filterQuery} onChange={event=>setFilterQuery(event.target.value)} placeholder="Search these records" autoFocus />}{visibleRows.length ? visibleRows.map(([title,meta,value,status,recordId,recordOwnerId,reviewState], index) => type === 'offices' ? <button className="office-directory-row" key={recordId} onClick={() => onSelectOffice(recordId)}><span className={`record-symbol ${status.toLowerCase()}`}><Briefcase /></span><span><strong>{title}</strong><small>{meta}</small></span><span className="record-value"><b>{value}</b><small>{status}</small></span><ArrowRight /></button> : type === 'team' ? <button className="office-directory-row" key={recordId} onClick={() => onSelectMember(recordId)}><span className={`record-symbol ${status.toLowerCase().replaceAll(' ','-')}`}><Users /></span><span><strong>{title}</strong><small>{meta}</small></span><span className="record-value"><b>{value}</b><small>{status}</small></span><ArrowRight /></button> : (type === 'orgmembers' || type === 'members' || (type === 'leaders' && reviewState !== 'leader-invite')) ? <button className="office-directory-row member-directory-row" key={recordId} onClick={() => setSelectedDirectoryMember([title,meta,value,status,recordId,recordOwnerId,reviewState])}><span className={`record-symbol ${status.toLowerCase().replaceAll(' ','-')}`}><Users /></span><span><strong>{title}</strong><small>{meta.split(' · ').at(-1)}</small></span><span className="record-value"><small>{status}</small></span><ArrowRight /></button> : <article key={`${title}-${index}`}><span className={`record-symbol ${status.toLowerCase().replaceAll(' ','-')}`}><Icon /></span><div><strong>{title}</strong><small>{meta}</small></div>{type === 'leaders' && reviewState === 'leader-invite' ? <span className="row-actions">{recordOwnerId && <button onClick={()=>decideLeaderInvitation(recordId,'approved')}>Approve</button>}<button className="danger" onClick={()=>decideLeaderInvitation(recordId,'revoked')}>Revoke</button></span> : type === 'plans' && recordScope === 'team' && !reviewState ? <button className="row-action" onClick={() => onReviewPlan(recordId)}>Review</button> : type === 'attendance' && recordScope === 'team' && status === 'absent' && reviewState === 'pending' ? <span className="row-actions"><button onClick={() => reviewAttendanceExcuse(recordId,'approved')}>Approve</button><button onClick={() => reviewAttendanceExcuse(recordId,'rejected')}>Reject</button></span> : type === 'orders' && recordOwnerId === user.id ? <span className="row-actions"><button onClick={() => onEditOrder(recordId)}>Edit</button>{status === 'active' ? <><button onClick={() => updateOrderStatus(recordId,'completed')}>Complete</button><button onClick={() => updateOrderStatus(recordId,'cancelled')}>Cancel</button></> : <button onClick={() => updateOrderStatus(recordId,'active')}>Reopen</button>}<button className="danger" onClick={()=>deleteOrder(recordId)}>Delete</button></span> : type === 'events' && status !== 'Done' ? <button className="row-action" onClick={() => completeEvent(recordId)}>Mark done</button> : type === 'anonymousinbox' ? <span className="row-actions">{status === 'new' && <button onClick={()=>updateAnonymousStatus(recordId,'read')}>Mark read</button>}{status !== 'resolved' && <button onClick={()=>updateAnonymousStatus(recordId,'resolved')}>Resolve</button>}</span> : type === 'feedbackinbox' && status !== 'resolved' ? <span className="row-actions"><button onClick={() => updateFeedbackStatus(recordId,'open')}>Open</button><button onClick={() => updateFeedbackStatus(recordId,'resolved')}>Resolve</button></span> : (type === 'notifications' || type === 'adminnotifications') ? <button className="row-action" onClick={() => openNotification(recordId,recordOwnerId)}>Open</button> : type === 'admindashboard' && status === 'Pending' ? <button className="row-action" onClick={() => approveMember(recordId)}>Approve</button> : (type === 'transfers' || type === 'transferapprovals') && status === 'Pending' && (user.role === 'Administrator' || user.role === 'Team leader' || user.isTeamLeader) ? <span className="row-actions"><button onClick={()=>decideTransfer(recordId,'approved')}>Approve</button><button onClick={()=>decideTransfer(recordId,'rejected')}>Reject</button></span> : <span className="record-value"><b>{value}</b><small>{status}</small></span>}</article>) : <div className="live-empty">{filterQuery ? 'No records match your search.' : type === 'admindashboard' ? 'No registrations are awaiting approval.' : type === 'offices' ? `No ${activeTab === 0 ? 'active' : 'archived'} offices.` : type === 'orders' && activeTab > 0 ? `No ${activeTab === 1 ? 'active' : 'completed'} orders yet.` : type === 'team' ? 'No active downlines yet. Members will appear here after selecting you as their sponsor.' : 'No live records yet. Use the action above to add the first one.'}</div>}</section>
    {selectedDirectoryMember && <div className="member-detail-backdrop" role="presentation" onClick={() => setSelectedDirectoryMember(null)}>
      <section className="member-detail-sheet" role="dialog" aria-modal="true" aria-labelledby="member-detail-title" onClick={event=>event.stopPropagation()}>
        <button className="member-detail-close" aria-label="Close member details" onClick={() => setSelectedDirectoryMember(null)}>×</button>
        <div className="member-detail-avatar"><Users /></div>
        <div><span>Member details</span><h2 id="member-detail-title">{selectedDirectoryMember[0]}</h2><p>{selectedDirectoryMember[1]}</p></div>
        <dl><div><dt>Rank</dt><dd>{selectedDirectoryMember[1].split(' · ').at(-1)}</dd></div><div><dt>Role</dt><dd>{selectedDirectoryMember[2]}</dd></div><div><dt>Status</dt><dd className={`member-status ${selectedDirectoryMember[3]}`}>{selectedDirectoryMember[3]}</dd></div></dl>
        {selectedDirectoryMember[4] === user.id ? <p className="member-self-note">This is your account. Your access cannot be changed from the office list.</p> : <>
          {user.role === 'Administrator' && selectedDirectoryMember[2].startsWith('Team leader') && <div className="rank-change-controls"><label htmlFor="leader-access">Team-leader access</label><select id="leader-access" value={leaderAccessSelection} onChange={event=>setLeaderAccessSelection(event.target.value)}><option value="limited">Limited access</option><option value="full">Full access</option></select><button onClick={changeLeaderAccess}>Update leader access</button><small>Limited: weekly orders and routine records. Full: member management, approvals, communication and reports.</small></div>}
          {pendingRankRequest && <div className="rank-request-card"><strong>Pending rank request</strong><span>{pendingRankRequest.old_rank} → {pendingRankRequest.new_rank}</span>{pendingRankRequest.reason && <p>{pendingRankRequest.reason}</p>}<div><button onClick={()=>decideRankChange(pendingRankRequest.id,'approved')}>Approve</button><button className="danger" onClick={()=>decideRankChange(pendingRankRequest.id,'rejected')}>Reject</button></div></div>}
          <div className="rank-change-controls"><label htmlFor="member-rank">Change rank</label><select id="member-rank" value={rankSelection} onChange={event=>setRankSelection(event.target.value)}>{rankOptions.map(rank=><option key={rank} value={rank}>{rank}</option>)}</select><label htmlFor="rank-reason">Reason or performance note <small>(optional)</small></label><textarea id="rank-reason" rows="2" value={rankReason} onChange={event=>setRankReason(event.target.value)} placeholder="Why is this rank changing?"/><button onClick={submitRankChange}>{user.role==='Administrator'?'Apply rank change':'Request rank change'}</button><small>{user.role==='Administrator'?'This applies immediately and notifies the member.':'The overall administrator must approve this request.'}</small></div>
          <div className="member-detail-actions">{selectedDirectoryMember[3] !== 'active' && <button onClick={() => setMemberStatus(selectedDirectoryMember[4],'active')}>Approve</button>}{selectedDirectoryMember[3] === 'active' && <button className="warning" onClick={() => setMemberStatus(selectedDirectoryMember[4],'suspended')}>Suspend</button>}{selectedDirectoryMember[3] === 'suspended' && <button onClick={() => setMemberStatus(selectedDirectoryMember[4],'pending')}>Set pending</button>}{user.role === 'Administrator' && <button className="danger" onClick={() => deleteMember(selectedDirectoryMember[4])}>Delete member</button>}</div>
        </>}
      </section>
    </div>}
...
const formData = {
  ...
  assignleader: {
    ...
    notice: 'Limited access handles weekly orders and routine office records. Full access also includes member management, approvals, communications and reports.',
    fields: [
      ['Registered person','Select a person','dynamic-member'],
      ['Office','Select an office','dynamic-office'],
      ['Access level','Limited access','leader-access'],
    ]
  },
  addoffice: {
  ...
  anonymousmessage: {
    title: 'Send anonymously', subtitle: 'Share a message without revealing who sent it', submit: 'Send anonymous message',
    notice: 'TeamFlow does not attach your account, name, email, rank or status. The originating office is retained so the right office leadership can respond appropriately.',
    fields: [
      ['Send to','Office team leader','anonymous-recipient'],
...
function FormPage({ type, onBack, selectedPlanId, selectedOrderId, selectedTransactionId, user }) {
  ...
  const submitForm = async event => {
    ...
    if (type === 'editprofile') {
    ...
    } else if (type === 'anonymousmessage') {
      const recipient=value('Send to'); const sendTo=recipient==='Both'?'both':recipient.includes('Overall')?'overall_director':'office_leader'
      ;({error}=await supabase.rpc('submit_anonymous_message',{message_body:value('Anonymous message'),send_to:sendTo}))
      ...
      ;({ error } = await supabase.rpc('admin_assign_team_leader_with_access', { target_user_id:value('Registered person'), target_office_id:value('Office'), requested_access_level:value('Access level').toLowerCase().startsWith('full') ? 'full' : 'limited' }))
...
function Placeholder({ page, user, onNavigate: navigatePage }) {
  ...
  const content = {
    ...
    more: ['More', 'Communication and account tools', [{icon: Bell,label:'Announcements',sub:'Team updates',color:'blue'},{icon: Sparkle,label:'Anonymous message',sub:'Identity-free office message',color:'navy'},{icon: Sparkle,label:'Suggestions',sub:'Share an idea',color:'amber'},{icon: CalendarCheck,label:'Events',sub:'Team activities',color:'green'},{icon: Sparkle,label:'Points & rewards',sub:'Your earned points',color:'navy'},{icon: Briefcase,label:'Change office',sub:'Request a transfer',color:'blue'},{icon: Users,label:'Profile & settings',sub:'Account details',color:'green'}]],
  }[page]
  ...
  if (page === 'more') content[2].unshift({ id:'learning', icon:BookOpen, label:'Learning Hub', sub:'Courses, lessons and your progress', color:'green' })
  if (page === 'more') content[2].unshift({ id:'creative', icon:Sparkle, label:'TIO Creative Hub', sub:'Fiverr Ecosystem · Design & Illustration', color:'navy' })
...
function AppShell({ user }) {
  ...
  const adminPages = new Set(['admindashboard','offices','officedetail','leaders','orgmembers','anonymousinbox','transfers','reports','auditlog','adminnotifications','adminsettings','roles','pointsettings','announcementtargeting'])
  const leaderPages = new Set(['leaderdashboard','weeklyorders','anonymousinbox','members','transferapprovals','teamreports','feedbackinbox','attendanceregister','bookreview','excusereview','planreview'])
  ...
  const navigate = destination => {
    const fullLeaderOnly = new Set(['members','anonymousinbox','transferapprovals','teamreports','feedbackinbox','bookreview','excusereview','planreview'])
    if (user.role !== 'Administrator' && user.isTeamLeader && user.leaderAccessLevel !== 'full' && fullLeaderOnly.has(destination)) return
  ...
  if (active === 'learning') return <LearningHub user={user} onBack={goBack} />
  if (active === 'creative') return <CreativeHub onBack={goBack} />
...
export default function App() {
  ...
  const loadAccount = async authUser => {
    ...
    const { data: profile } = await supabase
      .from('profiles')
      .select('full_name, role, rank, status, leader_access_level')
      .eq('id', authUser.id)
      .maybeSingle()
    ...
    setUser({ id: authUser.id, name: fullName, initials, role, rank: profile?.rank || 'Newbie', office, status: profile?.status || 'pending', isTeamLeader:Boolean(ledOffice), leaderAccessLevel:profile?.leader_access_level || (profile?.role === 'admin' ? 'full' : 'limited'), ledOffice:ledOffice?.name || null, ledOfficeId:ledOffice?.id || null })
