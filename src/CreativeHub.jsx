...
const tools = [
  ...
  { id: 'illustration', name: 'StoryWeaver Studio', description: 'Create character reference sheets, direct scenes, and arrange your storybook.', fields: [] },
]
export default function CreativeHub({ onBack }) {
  ...
  if (selected?.id === 'illustration') return <IllustrationStudio initialProjectId={draftId} onBack={() => { setSelected(null); setRevision(n => n + 1) }} />
  return <main className="learning-hub"><header className="learning-heading"><button onClick={back} disabled={busy}>← Back</button><span>TeamFlow · TIO Creative Hub</span></header>
    ...
    {selected ? <form className="learning-editor" onSubmit={save}>
      ...
      {selected.fields.map(field => <label key={field}>{field}<textarea rows={3} maxLength={30000} value={brief[field] || ''} onChange={e => { setBrief(previous => ({ ...previous, [field]: e.target.value })); setDirty(true) }} /></label>)}
