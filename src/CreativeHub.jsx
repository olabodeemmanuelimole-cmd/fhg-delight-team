...
import IllustrationSetup from './IllustrationSetup'
import IllustrationCanvas from './IllustrationCanvas'
import { validateIllustration } from './illustrationStyles'
...
export default function CreativeHub({ onBack }) {
  ...
  async function save(e) {
    ...
    try {
      ...
      if (selected.id === 'illustration') { const problem = validateIllustration(brief); if (problem) throw new Error(problem) }
      const { data: auth, error: authError } = await supabase.auth.getUser()
      if (authError) throw authError
  ...
  function download() {
    const content = '# ' + (title || selected.name) + '\n\nTool: ' + selected.name + '\n\nProject brief — not AI-generated output.\n\n' + Object.entries(brief).map(([field, value]) => '## ' + field + '\n' + (typeof value === 'object' ? JSON.stringify(value, null, 2) : value || 'Not supplied') + '\n').join('\n')
  ...
  return <main className="learning-hub"><header className="learning-heading"><button onClick={back} disabled={busy}>← Back</button><span>TeamFlow · TIO Creative Hub</span></header>
    ...
    <p className="learning-notice">Project planning is available. Illustration generation is an administrator-only test requiring backend setup. AI writing and manuscript analysis are not connected yet.</p>
    ...
    {selected ? <form className="learning-editor" onSubmit={save}>
      ...
      {selected.id === 'illustration' && <IllustrationSetup brief={brief} onChange={changes => { setBrief(previous => ({ ...previous, ...changes })); setDirty(true) }} />}
      {(selected.id === 'illustration' ? ['Book title and audience', 'Book dimensions and format', 'Colour direction', ...((brief.creationMode || 'manual') === 'manual' ? ['Characters and locked features', 'Recurring objects and locations', 'Scene plan', 'Continuity rules'] : [])] : selected.fields).map(field => <label key={field}>{field}<textarea rows={3} maxLength={30000} value={brief[field] || ''} onChange={e => { setBrief(previous => ({ ...previous, [field]: e.target.value })); setDirty(true) }} /></label>)}
      ...
      {selected.id === 'illustration' && draftId && <IllustrationCanvas key={draftId} projectId={draftId} dirty={dirty} />}
